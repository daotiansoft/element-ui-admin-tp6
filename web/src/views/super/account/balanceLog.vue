        ...
        <el-table v-loading="loading" :data="items" style="width: 100%" empty-text="暂无数据"
          ...
          <el-table-column prop="type" label="类型" width="100" align="center">
            <template slot-scope="scope">
              ...
              <el-tag v-if="scope.row.type === 'out'" type="danger">支出</el-tag>

              <el-tag v-if="scope.row.type === 'recharge'">充值</el-tag>

              <el-tag v-if="scope.row.type === 'withdrawal'" type="danger">提现</el-tag>
              <el-tag v-if="scope.row.type === 'withdrawal_fail'">提现失败</el-tag>
            </template>
          </el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      load_items() {
        this.loading = true
        this.$store
          .dispatch('super/balanceLog/items', this.search_form)
          .then((res) => {
      ...
      del() {
        ...
        this.$confirm('此操作将永久删除选中数据, 是否继续?', '提示', {
          ...
          .then(() => {
            ...
            this.loading = true
            this.$store
              .dispatch('super/balanceLog/del', { ids: ids })
              .then((res) => {
                this.$message({
                  message: res,
                  type: 'success'
