        ...
        >
          ...
          <el-table-column label="状态" width="150">
            <template slot-scope="scope">
              <el-tag v-if="scope.row.status === 1" disable-transitions>
                正常
              </el-tag>
...
export default {
  ...
  methods: {
    ...
    load_items() {
      this.loading = true
      this.$store
        .dispatch('super/articleCate/items', this.search_form)
        .then((res) => {
    ...
    submit_add() {
      this.$refs.add_form.validate((valid) => {
        if (valid) {
          ...
          this.$store
            .dispatch('super/articleCate/add', this.add_form)
            .then((res) => {
              this.$message({
                message: res,
                type: 'success'
    ...
    submit_edit() {
      this.$refs.edit_form.validate((valid) => {
        if (valid) {
          this.form_loading = true
          this.$store
            .dispatch('super/articleCate/edit', this.edit_form)
            .then((res) => {
              this.$message({
                message: res,
                type: 'success'
